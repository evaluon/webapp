...
.controller('EvaluationInstitutesCtrl', function(
    $scope, evaluationInstitutes, $alert, $state
){
    $scope.rowInstitutes = [];
    $scope.routes ={
    ...
    evaluationInstitutes.getAll().then(function(success){
        if(success){
            ...
            if(success.data.data == []){
                $alert.show('Usted no tiene instituciones activas, por favor comuníquese con su maestro para solucionar el problema');
                $state.go('home');
            }
